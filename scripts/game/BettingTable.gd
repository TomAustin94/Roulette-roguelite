...
func _draw_cell(
	...
	if has_bet:
		...
		draw_circle(chip_pos, 10.0, chip_col)
		draw_arc(chip_pos, 10.0, 0.0, TAU, 16, Color(0.0,0.0,0.0,0.4), 1.5)
		var amt_str  := str(amount)
		var asize    := font.get_string_size(amt_str, HORIZONTAL_ALIGNMENT_LEFT, -1, 9)
		draw_string(font, chip_pos - asize * 0.5 + Vector2(0, asize.y * 0.3),
					amt_str, HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color.BLACK)
